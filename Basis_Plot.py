...
def Basis_Plot():
    order_p = 4 #Fra oppgave: the order is p + 1, where p is the polynomial degree of the spline
    # create a set of B-spline basis functions
    basis1 = sp.BSplineBasis(order=order_p, knots=[0, 0, 0, 1, 2, 3, 4, 4, 4])
    basis2 = sp.BSplineBasis(order=order_p, knots=[0, 0, 0, 1, 1, 2, 2, 3, 3, 4, 4, 4])
    basis3 = sp.BSplineBasis(order=order_p, knots=[0, 0, 0, 0, 1, 2, 3, 4, 4, 4, 4])
    basis4 = sp.BSplineBasis(order=order_p, knots=[0, 0, 0, 0, 1, 1, 2, 2, 3, 3, 4, 4, 4, 4])
